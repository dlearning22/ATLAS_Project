...
#############################################################################################
        ...
        self.Att4 = Attention_block(F_g= planes[2],F_l= planes[2],F_int= planes[1])
        
        self.Att3 = Attention_block(F_g= planes[1],F_l= planes[1],F_int= planes[0])
       
        self.Att2 = Attention_block(F_g= planes[0],F_l= planes[0],F_int= 16)
...
def forward(self, x):
        e0 = self.ec0(x) #32
        e1 = self.ec1_2(self.ec1(self.maxpool(e0))) #64
        e2 = self.ec2_2(self.ec2(self.maxpool(e1))) #128
        e3 = self.ec3_2(self.ec3(self.maxpool(e2))) #256
        d3 = self.up3(self.dc3_2(self.dc3(e3))) #128
        a4 = self.Att4(g= d3, x= e2 )  
        if d3.size()[2:] != a4.size()[2:]:
            d3 = F.interpolate(d3,
                               a4.size()[2:],
                               mode='trilinear',
                               align_corners=False)
        d3 = torch.cat((d3, a4), 1) #256
        d2 = self.up2(self.dc2_2(self.dc2(d3))) #64
        a3 = self.Att3(g= d2, x= e3 ) #64
